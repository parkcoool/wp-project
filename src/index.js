...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const controlSwitchInputs = document.querySelectorAll(
  ...
  const syncAudioMuteButton = (button) => {
    ...
    button.setAttribute(
      'aria-label',
      setting.muted
        ? `Unmute ${audioControl} volume`
        : `Mute ${audioControl} volume`,
    );
  ...
  const syncSkillKeyDisplays = () => {
    skillKeyInputs.forEach((input) => {
      ...
      const keyLabel =
        input.value ||
        input.dataset.currentKey ||
        input.dataset.defaultKey ||
        '';
  ...
  document
    .querySelector('.control-reset-btn')
  ...
  const gameplayDropdowns = document.querySelectorAll(
    '.gameplay-setting-dropdown',
  );
  ...
  document
    .getElementById('pause-continue-btn')
    ?.addEventListener('click', () => {
      resumeGame();
    });
  ...
  document
    .getElementById('gameover-menu-btn')
    ?.addEventListener('click', () => {
      exitGameToMenu();
    });
  ...
  document
    .getElementById('stageclear-menu-btn')
    ?.addEventListener('click', () => {
      exitGameToMenu();
    });
...
const typeMainSystemDescription = () => {
