...
export function registerFuelCallbacks(callbacks) {
  ...
  if (callbacks.triggerGameOver)
    _cb.triggerGameOver = callbacks.triggerGameOver;
}
 ...
 */
const _clampFuel = () => {
  GameState.fuel.current = Math.max(
    0,
    Math.min(GameState.fuel.max, GameState.fuel.current),
  );
};
...
const _updateFuelUI = () => {
  const pct =
    GameState.fuel.max > 0
      ? Math.round((GameState.fuel.current / GameState.fuel.max) * 100)
      : 0;
  ...
  if (arc) {
    arc.setAttribute(
      'stroke',
      pct <= FUEL_WARNING_THRESHOLD ? '#FF7A1A' : '#FFC857',
    );
    const arcLen = 251.3;
